...
export function normalize(node: SvgXmlNode, parentMatrix: Matrix = IDENTITY): SvgXmlNode {
  ...
  const parsedStyle = node.attributes.style ? parseStyleAttribute(node.attributes.style) : {};
  const attr = {
    ...node.attributes,
    ...parsedStyle,
  ...
  const newChildren = node.children.map(child => normalize(child, localMatrix));
