...
export function ScreenSetSelector() {
  ...
  const [ createScreenSetModalOpen, setCreateScreenSetModalOpen ] = useState(false);
  ...
  useEffect(() => {
    screenSets
      ...
      .forEach(screenSet => {
        invoke<ArrayBuffer>("get_screen_img", { id: screenSet.screenImgId })
          .then(buf => {
  ...
  const handleRename = (id: string, name: string) => {
    invoke<ScreenSetMeta[]>("rename_screen_set", { id, name })
      .then(setScreenSets);
  };
  ...
  const createScreenSet = (name: string) => {
    invoke<ScreenSet>("create_screen_set", { name })
      .then(ss => nav(`/creator/${ss.id}`));
  };
...
function ScreenSetItem({ screenSet, imgUrl, onEdit, onRename, onDelete }: ScreenSetItemProps) {
...
function DeleteConfirmation({ open, onClose, onConfirm }: DeleteConfirmationProps) {
  return (
    <Modal
      open={open}
      onClose={onClose}
      ...
      footer={
        <div className="row align-items-center justify-content-center gap-12">
          ...
          <button onClick={onConfirm} style={{ background: "rgb(255,0,59)" }}>Delete</button>
        </div>
      }
    >
      <div>
        Are you sure you want to permanently delete this Screen Set?
      </div>
    </Modal>
  );
}
    ...
    <Modal
      ...
      footer={
        <div className="row align-items-center justify-content-center gap-16">
          <button onClick={onClose}>Cancel</button>
          <button onClick={() => onConfirm(proposedName)} disabled={!proposedName}>Create</button>
        </div>
      }
    >
      <div>
        <p>
          Enter a name for the new ScreenSet:
        </p>
        <input
          value={proposedName}
